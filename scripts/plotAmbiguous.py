...
def main():
    ...
    plot_ambiguous_alleles_tables_from_folder(
...
def plot_ambiguous_alleles_tables_from_folder(
    ...
    Note that each ambiguous read is assigned to a reference (usually the first one) and
        and mutations/indels are plotted in relation to this reference sequence.
    ...
    for ref_name in ref_names:
        ...
        for ind, sgRNA in enumerate(sgRNA_sequences):
            ...
            df_alleles_around_cut = CRISPRessoShared.get_dataframe_around_cut_asymmetrical(
                df_alleles.loc[df_alleles["Reference_Name"] == ambiguous_ref_name], cut_point, plot_half_window, plot_half_window
            )
