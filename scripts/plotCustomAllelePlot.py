...
def main():
    ...
    parser.add_argument(
        "--use_matplotlib", action="store_true", help="Use matplotlib for plotting instead of plotly/d3 when CRISPRessoPro is installed"
    )
    ...
    plot_alleles_tables_from_folder(
        args.CRISPResso2_folder,
        args.output_root,
        MIN_FREQUENCY=args.min_freq,
        MAX_N_ROWS=args.max_rows,
        SAVE_ALSO_PNG=args.save_png,
        plot_cut_point=args.plot_cut_point,
        plot_left=args.plot_left,
        plot_right=args.plot_right,
...
def get_dataframe_around_cut_asymmetrical(df_alleles, cut_point, plot_left, plot_right, collapse_by_sequence=True):
    ...
    if cut_point + plot_right + 1 > len(ref1):
        raise (
            CRISPRessoShared.BadParameterException(
                "The plotting window cannot extend past the end of the amplicon. Amplicon length is "
                + str(len(ref1))
                + " but plot extends to "
                + str(cut_point + plot_right + 1)
            )
        )

    df_alleles_around_cut = pd.DataFrame(
        list(df_alleles.apply(lambda row: get_row_around_cut_asymmetrical(row, cut_point, plot_left, plot_right), axis=1).values),
        columns=["Aligned_Sequence", "Reference_Sequence", "Unedited", "n_deleted", "n_inserted", "n_mutated", "#Reads", "%Reads"],
    )

    df_alleles_around_cut = (
        df_alleles_around_cut.groupby(["Aligned_Sequence", "Reference_Sequence", "Unedited", "n_deleted", "n_inserted", "n_mutated"])
        .sum()
        .reset_index()
