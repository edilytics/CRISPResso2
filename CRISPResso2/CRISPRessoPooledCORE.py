    ...
    try:
        ...
        if len(sys.argv) == 1:
            raise CRISPRessoShared.BadParameterException(
                CRISPRessoShared.format_cl_text(
                    "usage: CRISPRessoPooled [-r1 FASTQ_R1] [-r2 FASTQ_R2] [-f AMPLICONS_FILE] [-x GENOME_ROOT] [-n NAME]\n"
                    ...
                    + "-f AMPLICONS_FILE     Tab-separated file containing information for "
                    "each amplicon, including at least columns for the amplicon_name and "
                    "amplicon_seq (default: None)\n"
                    + "-x GENOME_ROOT        Folder that contains the bowtie2-indexed genome "
                    "for optional unbiased alignment of reads (default: None, reads are only "
                    "aligned to provided amplicon sequences)\n"
                    + "-n NAME, --name NAME  Name for the analysis (default: name based on "
                    "input file name)\n"
                )
            )
...
|_______________________|
        ...
        options_to_ignore = {
            ...
            "bowtie2_index",
        }  # these options will be set for each sub-run or should be not be passed to sub-runs because they are pooled-specific
        ...
        if args.aligned_pooled_bam is not None:
            ...
            if args.bowtie2_index == "":
                raise CRISPRessoShared.BadParameterException(
                    ...
                    "the bowtie2 index file with the parameter -x or --bowtie2_index."
                )
        ...
        if args.amplicons_file and not args.bowtie2_index:
            ...
            info(
                ...
                "performed using only the provided amplicon sequences."
            )

        elif args.bowtie2_index and not args.amplicons_file:
            RUNNING_MODE = "ONLY_GENOME"
            info(
                "Only the bowtie2 reference genome index file was provided. The analysis "
                "will be performed using only genomic regions with sufficient read "
                "alignment depth."
            )
        elif args.bowtie2_index and args.amplicons_file:
            RUNNING_MODE = "AMPLICONS_AND_GENOME"
            info(
                "Amplicon description file and bowtie2 reference genome index files "
                "provided. The analysis will be performed using the reads that are aligned "
                "to the genome only where the provided amplicons align and not to other "
                "genomic regions."
            )
        else:
            error(
                    ...
                    for arg in vars(args):
                        ...
                        if arg not in vars(previous_run_data["running_info"]["args"]):
                            ...
                            info(
                                ...
                                + str(arg)
                                + " = "
                                + str(getattr(previous_run_data["running_info"]["args"], arg))
                                + "\n\tnew argument: "
                                + str(arg)
                                + " = "
        ...
        # read filtering (for quality) is done at the individual crispresso run
            ...
            if not args.trim_sequences:
                ...
                CRISPRessoShared.write_crispresso_info(crispresso2_info_file, crispresso2_info)
        ...
        default_input_amplicon_headers = [
            "amplicon_name",
            "amplicon_seq",
            "guide_seq",
        ...
        if RUNNING_MODE in {"ONLY_AMPLICONS", "AMPLICONS_AND_GENOME"}:
            ...
            debug(f"Header variable names in order: {headers}")
            ...
            df_template["amplicon_name"] = df_template["amplicon_name"].astype(str)
            ...
            if not len(df_template.amplicon_seq.unique()) == df_template.shape[0]:
                ...
                raise CRISPRessoShared.BadParameterException(
                    "The amplicon sequences must be distinct! (Duplicated entries: " + str(duplicated_entries.values) + ")"
                )

            # check to see that no sequences and their reverse complements are present
            # Beware, this is a numpy array of dtype str and if you add these arrays
            # amp_seqs + rc_amp_seqs, it will concat the strings, not the arrays....
            ...
            for seq in amp_seqs:
                if seq in rc_amp_seqs:
                    raise Exception(
                        "Amplicon sequences must be distinct! The amplicon sequence %s is "
                        "the reverse complement of another amplicon sequence in the region "
                        "file. Please provide only one of the two sequences."
                        % seq
                    )
            ...
            if not len(df_template.amplicon_name.unique()) == df_template.shape[0]:
                ...
                raise CRISPRessoShared.BadParameterException(
                    "The amplicon names must be distinct! (Duplicated names: " + str(duplicated_entries.values) + ")"
                )
            ...
            df_template["run_name"] = run_names
            ...
            for idx, row in df_template.iterrows():
                ...
                if "guide_seq" in df_template.columns and not pd.isnull(row.guide_seq):
                    ...
                    if not cut_points:
                        warn(
                            "\nThe guide sequence/s provided: %s is(are) not present in the "
                            "amplicon sequence:%s! \nNOTE: The guide will be ignored for the "
                            "analysis. Please check your input!"
                            % (row.guide_seq, row.amplicon_seq)
                        )
        ...
        if RUNNING_MODE == "ONLY_AMPLICONS":
            ...
            aligner_command = "bowtie2 -x %s -p %s %s -U %s 2>>%s | samtools view -bS - > %s" % (
            ...
            if args.limit_open_files_for_demux:
                ...
                for bam_line in bam_iter:
                    ...
                    if len(bam_line_els) < 9:
                        if args.debug:
                            info(
                                "ERROR got unexpected line from bam: {0} with els: {1}".format(
                                    bam_line,
                                    str(bam_line_els),
                                )
                            )
            ...
            info("Demultiplexing reads and running CRISPResso on each amplicon...")
            ...
            df_template["n_reads"] = df_template["n_reads"].astype(int)
            df_template["n_reads_aligned_%"] = df_template["n_reads"] / float(N_READS_ALIGNED) * 100

            df_template["crispresso_command"] = ""
            df_template["crispresso_output_folder"] = ""

            # get count of rows with > args.min_reads_to_use_region
            count_good_runs = df_template[df_template["n_reads"] >= args.min_reads_to_use_region].shape[0]
            args.n_processes = 1
            if count_good_runs > 0 and int(n_processes_for_pooled / count_good_runs) > 1:
                args.n_processes = int(n_processes_for_pooled / count_good_runs)
            debug("Running sub-CRISPResso runs with %d processes each" % args.n_processes)

            for idx, row in df_template.iterrows():
                info("Processing: %s with %d reads" % (idx, row.n_reads))
                if row["n_reads"] > args.min_reads_to_use_region:
                    info("The amplicon [%s] has enough reads (%d) mapped to it! Running CRISPResso!\n" % (idx, row.n_reads))

                    this_run_args_from_amplicons_file = {}
                    for column_name in default_input_amplicon_headers:
                        if (
                            (column_name in df_template.columns)
                            and (row[column_name])
                            and (not pd.isnull(row[column_name]) and (not str(row[column_name]) == "NA"))
                        ):
                            this_run_args_from_amplicons_file[column_name] = row[column_name]

                    this_run_name = row["run_name"]
                    crispresso_cmd = 'CRISPResso -r1 %s -a %s %s -o %s --name "%s" --display_name "%s"' % (
                    ...
                    # first, set the general CRISPResso options from args for this sub-run (e.g. plotting options, etc)
    # # note that the crispresso_options_for_pooled doesn't include e.g. amplicon_seq so when someone calls CRISPRessoPooled with -a that won't
    # get passed on here
                    crispresso_cmd = CRISPRessoShared.overwrite_crispresso_options(
                        cmd=crispresso_cmd, tool="Core", option_names_to_overwrite=crispresso_options_for_pooled, option_values=args
                    )
                    # next set the per-amplicon options we read from the Amplicons file (and are stored in this_run_args_from_amplicons_file)
                    crispresso_cmd = CRISPRessoShared.overwrite_crispresso_options(
                        cmd=crispresso_cmd,
                        tool="Core",
                        option_names_to_overwrite=default_input_amplicon_headers,
                        option_values=this_run_args_from_amplicons_file,
                    )

                    # remove the CRISPResso command from the beginning and replace with
                    # args.crispresso_command (in case it contains spaces)
                    crispresso_cmd = (
                    ...
                    df_template.at[idx, "crispresso_command"] = crispresso_cmd
                    df_template.at[idx, "crispresso_output_folder"] = os.path.join(OUTPUT_DIRECTORY, "CRISPResso_on_%s" % this_run_name)

                else:
                    warn("Skipping amplicon [%s] because too few reads (%d) align to it\n" % (idx, row.n_reads))

            CRISPRessoMultiProcessing.run_crispresso_cmds(
                crispresso_cmds, n_processes_for_pooled, "amplicon", args.skip_failed, start_end_percent=(16, 80)
            )
            ...
            if can_finish_incomplete_run and "mapping_amplicons_to_reference_genome" in crispresso2_info["running_info"]["finished_steps"]:
                ...
                with open(filename_aligned_amplicons_sam, encoding="utf-8") as aln:
                    for line in aln.readlines():
                        ...
                        if this_run_name not in df_template.run_name.values:
                            info("The amplicon [%s] is not present in the amplicon file provided!" % this_run_name)
                            continue
                        this_amplicon_name = df_template.loc[df_template.run_name == this_run_name].index[
                            0
                        ]  # amplicon name may have spaces and other bad characters
                        if line_els[2] == "*":
                            info("The amplicon [%s] is not mappable to the reference genome provided!" % this_amplicon_name)
                            additional_columns.append([this_amplicon_name, "NOT_ALIGNED", 0, -1, "+", ""])
                        else:
                            ...
                            additional_columns.append([this_amplicon_name, line_els[2], seq_start, seq_stop, strand, line_els[9]])
                            info("The amplicon [%s] was mapped to: %s:%d-%d " % (this_amplicon_name, line_els[2], seq_start, seq_stop))
                additional_columns_df = pd.DataFrame(
                    additional_columns, columns=["amplicon_name", "chr_id", "bpstart", "bpend", "strand", "reference_seq"]
                ).set_index("amplicon_name")
        ...
        if RUNNING_MODE in {"ONLY_GENOME", "AMPLICONS_AND_GENOME"}:
            bam_filename_genome = _jp(
                "{0}_GENOME_ALIGNED.bam".format(
                    normalize_name(
                        args.name,
                        args.fastq_r1,
                        args.fastq_r2,
                        args.aligned_pooled_bam,
                    )
                )
            )
            ...
            # otherwise, align reads to the genome and count reads
                ...
                aligner_command = "bowtie2 -x %s -p %s %s -U %s 2>>%s| samtools view -bS - | samtools sort -@ %d - -o %s" % (
                    args.bowtie2_index,
                    n_processes_for_pooled,
                    bowtie2_options_string,
                    processed_output_filename,
            ...
            if (
                can_finish_incomplete_run
                and "genome_demultiplexing" in crispresso2_info["running_info"]["finished_steps"]
                and os.path.isfile(REPORT_ALL_DEPTH)
            ):
                info("Using previously-computed demultiplexing of genomic reads")
                df_all_demux = pd.read_csv(REPORT_ALL_DEPTH, sep="\t")
                df_all_demux["loc"] = (
                    df_all_demux["chr_id"].apply(str) + " " + df_all_demux["start"].apply(str) + " " + df_all_demux["end"].apply(str)
                )
                ...
                if RUNNING_MODE == "AMPLICONS_AND_GENOME" and not args.demultiplex_genome_wide:
                    s1 = (
                        rf"samtools view -F {args.samtools_exclude_flags} "
                        rf"{bam_filename_genome} "
                        rf"__REGIONCHR__:__REGIONSTART__-__REGIONEND__ 2>>{log_filename} |"
                        + r"""awk 'BEGIN{OFS="\t";num_records=0;"""
                        r"""fastq_filename="__OUTPUTPATH__REGION___REGIONCHR_____REGIONSTART_____REGIONEND__.fastq";} \
                        { \
                            print "@"$1"\n"$10"\n+\n"$11 > fastq_filename; \
                            num_records++; \
                        } \
                    ...
                    for idx, row in df_template.iterrows():
                        ...
                        sub_chr_command = (
                            ...
                            .replace("__REGIONSTART__", str(row.bpstart))
                            .replace("__REGIONEND__", str(row.bpend))
                            .replace("__DEMUX_CHR_LOGFILENAME__", chr_output_filename)
                        )
                        if (
                            chr_output_filename not in chr_output_filenames
                        ):  # sometimes multiple amplicons map to the same region so we don't want the region to be written to by multiple processes
                ...
                # if we should demultiplex everwhere (not just where amplicons aligned)
                    ...
                    s1 = (
                        rf"""samtools view -F {args.samtools_exclude_flags} {bam_filename_genome} __CHR____REGION__ 2>>{log_filename} |"""
                    ...
                    for chr_str in chrs:
                        ...
                        if N_READS_ALIGNED > 10000000 and chr_len > chr_step_size * 2:
                            ...
                            while curr_end < chr_len:
                                ...
                                sub_chr_command = chr_cmd.replace("__REGION__", ":%d-%d " % (curr_pos, curr_end)).replace(
                                    "__DEMUX_CHR_LOGFILENAME__", chr_output_filename
                                )
                                chr_commands.append(sub_chr_command)
                                chr_output_filenames.append(chr_output_filename)
                                curr_pos = curr_end
                            ...
                            if curr_end < chr_len:
                                ...
                                sub_chr_command = chr_cmd.replace("__REGION__", ":%d-%d " % (curr_pos, chr_len)).replace(
                                    "__DEMUX_CHR_LOGFILENAME__", chr_output_filename
                                )
                                chr_commands.append(sub_chr_command)
                                chr_output_filenames.append(chr_output_filename)
                ...
                CRISPRessoMultiProcessing.run_parallel_commands(
                    chr_commands, n_processes=n_processes_for_pooled, descriptor="Demultiplexing reads by location", continue_on_fail=args.skip_failed
                )
        ...
        if RUNNING_MODE == "AMPLICONS_AND_GENOME":
            ...
            if can_finish_incomplete_run and "crispresso_amplicons_and_genome" in crispresso2_info["running_info"]["finished_steps"]:
                ...
                (n_reads_aligned_genome, fastq_region_filenames, files_to_match) = crispresso2_info["running_info"]["finished_steps"][
                    "crispresso_amplicons_and_genome"
                ]
            else:
                # set number of processes for CRISPResso runs based on the number of regions with sufficient reads
                count_good_runs = df_all_demux[df_all_demux["number of reads"] >= args.min_reads_to_use_region].shape[0]
                args.n_processes = 1
                if count_good_runs > 0 and int(n_processes_for_pooled / count_good_runs) > 1:
                    args.n_processes = int(n_processes_for_pooled / count_good_runs)
                debug("Running sub-CRISPResso runs with %d processes each" % args.n_processes)
                ...
                for idx, row in df_template.iterrows():
                    ...
                    if demux_key in df_all_demux.index:
                        ...
                        elif fastq_filename_region in files_to_match:
                        ...
                        if N_READS >= args.min_reads_to_use_region and fastq_filename_region != "":
                            ...
                            this_run_name = row["run_name"]
                            crispresso_cmd = 'CRISPResso -r1 %s -o %s --name "%s" --display_name "%s"' % (
                                fastq_filename_region,
                                OUTPUT_DIRECTORY,
                                this_run_name,
                                idx,
    ...
    # get passed on here
                            ...
                            crispresso_cmd = (
                                args.crispresso_command + " " + " ".join(crispresso_cmd.split()[1:])
                            )  # set the crispresso_command in case it had spaces and would be hard to parse
                ...
                CRISPRessoMultiProcessing.run_crispresso_cmds(
                    crispresso_cmds, n_processes_for_pooled, "amplicon", args.skip_failed, start_end_percent=(15, 85)
                )

                crispresso2_info["running_info"]["finished_steps"]["crispresso_amplicons_and_genome"] = (
                    n_reads_aligned_genome,
                    fastq_region_filenames,
                    files_to_match,
                )
            ...
            if args.skip_reporting_problematic_regions:
                ...
                if can_finish_incomplete_run and "reporting_problematic_regions" in crispresso2_info["running_info"]["finished_steps"]:
                    ...
                    coordinates = CRISPRessoMultiProcessing.run_function_on_array_chunk_parallel(
                        summarize_region_fastq_input, summarize_region_fastq_chunk, n_processes=n_processes_for_pooled
                    )
        ...
        if RUNNING_MODE == "ONLY_GENOME":
            ...
            if (
                can_finish_incomplete_run
                and "demultiplexing_genome_only_regions" in crispresso2_info["running_info"]["finished_steps"]
                and os.path.exists(filename_reads_aligned_to_genome_only)
            ):
                ...
                # Create unique run names for each region
                seen_runnames = {}
                run_names = []
                ...
                for idx, row in df_regions.iterrows():
                    ...
                    this_index = 0
                    while this_run_name in seen_runnames:
                        this_run_name = CRISPRessoShared.clean_filename(this_unclean_name) + "_" + str(this_index)
                        this_index += 1
                    seen_runnames[this_run_name] = 1
                    run_names.append(this_run_name)
                    run_display_names.append(this_unclean_name)
                df_regions["run_name"] = run_names
                df_regions["run_display_name"] = run_display_names
            ...
            if can_finish_incomplete_run and "crispresso_genome_only" in crispresso2_info["running_info"]["finished_steps"]:
                ...
                # set number of processes for CRISPResso runs based on the number of regions with sufficient reads
                count_good_runs = df_regions[df_regions["n_reads"] >= args.min_reads_to_use_region].shape[0]
                args.n_processes = 1
                if count_good_runs > 0 and int(n_processes_for_pooled / count_good_runs) > 1:
                    args.n_processes = int(n_processes_for_pooled / count_good_runs)
                debug("Running sub-CRISPResso runs with %d processes each" % args.n_processes)
                ...
                for idx, row in df_regions.iterrows():
                    if row.n_reads > args.min_reads_to_use_region:
                        ...
                        this_run_name = row.run_name
                        this_run_display_name = row.run_display_name

                        if pd.isna(row.sequence):
                            raise CRISPRessoShared.BadParameterException(
                                "Cannot extract sequence " + str(row.sequence) + " from input reference " + uncompressed_reference
                            )

                        crispresso_cmd = 'CRISPResso -r1 %s -a %s -o %s -n "%s" --display_name "%s"' % (
                            row.fastq_file,
                            row.sequence,
                            OUTPUT_DIRECTORY,
                            this_run_name,
                            this_run_display_name,
                        )
                        crispresso_cmd = CRISPRessoShared.overwrite_crispresso_options(
                            cmd=crispresso_cmd,
                            option_names_to_overwrite=crispresso_options_for_pooled,
                            option_values=args,
                            tool="Core",
                        )
                        # remove the CRISPResso command from the beginning and replace with
                ...
                CRISPRessoMultiProcessing.run_crispresso_cmds(
                    crispresso_cmds, n_processes_for_pooled, "region", args.skip_failed, start_end_percent=(15, 85)
                )
        ...
        for idx, row in df_final_data.iterrows():
            ...
            try:
                ...
                for ref_name in run_data["results"]["ref_names"]:  # multiple alleles could be provided
                    ...
                    n_insertion_and_deletion_and_substitution += run_data["results"]["alignment_stats"][
        ...
        if args.crispresso1_mode:
            ...
            df_summary_quantification.fillna("NA").to_csv(
                samples_quantification_summary_filename, sep="\t", index=None, columns=crispresso1_columns, header=crispresso1_print_columns
            )
        else:
            df_summary_quantification.fillna("NA").to_csv(samples_quantification_summary_filename, sep="\t", index=None)

        crispresso2_info["results"]["alignment_stats"]["samples_quantification_summary_filename"] = os.path.basename(
        ...
        crispresso2_info["results"]["all_region_display_names"] = all_region_display_names
        ...
        if not args.suppress_plots:
            ...
            CRISPRessoPlot.plot_reads_total(
                ...
                save_png=save_png,
                cutoff=args.min_reads_to_use_region,
            )
            ...
            crispresso2_info["results"]["general_plots"]["summary_plot_datas"][plot_name] = [
                ("CRISPRessoPooled summary", os.path.basename(samples_quantification_summary_filename))
            ]
        ...
        if args.compile_postrun_references:
            ...
            pd.DataFrame(postrun_references, columns=["region_name", "reference_seqs", "reference_names", "reference_frequencies"]).to_csv(
                postrun_reference_file, sep="\t", index=False
            )
