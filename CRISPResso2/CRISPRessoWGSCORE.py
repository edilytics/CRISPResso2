        ...
        if len(sys.argv) == 1:
            raise CRISPRessoShared.BadParameterException(
                CRISPRessoShared.format_cl_text(
                    "usage: CRISPRessoWGS [-b BAM_FILE] [-f REGION_FILE] [-r REFERENCE_FILE] [-n NAME]\n"
...
|____________|
        ...
        options_to_ignore = {
            ...
            "bowtie2_index",
        }  # these options will be set for each sub-run or should be not be passed to sub-runs because they are wgs-specific
        ...
        df_regions = pd.read_csv(
            args.region_file,
            names=["chr_id", "bpstart", "bpend", "Name", "sgRNA", "Expected_HDR", "Coding_sequence"],
            comment="#",
            sep="\t",
            dtype={"Name": str, "chr_id": str},
        )
        ...
        df_regions["bam_file_with_reads_in_region"], df_regions["fastq_file_trimmed_reads_in_region"], df_regions["row_fastq_exists"] = zip(
            *df_regions.apply(set_filenames, axis=1), strict=False
        )
        ...
        if (
            ...
            and num_rows_without_fastq == 0
            and os.path.isfile(report_reads_aligned_filename)
            and "generation_of_fastq_files_for_each_amplicon" in crispresso2_info["running_info"]["finished_steps"]
        ):
            ...
            df_regions.infer_objects(copy=False).fillna("NA").to_csv(
                report_reads_aligned_filename, sep="\t", columns=cols_to_print, index_label="Name"
            )
        ...
        for idx, row in df_regions.iterrows():
            if row["n_reads"] >= args.min_reads_to_use_region:
                ...
                crispresso_cmd = CRISPRessoShared.overwrite_crispresso_options(
                    cmd=crispresso_cmd, option_names_to_overwrite=crispresso_options_for_wgs, option_values=args
                )
        ...
        for idx, row in df_regions.iterrows():
            ...
            if failed_run_bool:
                ...
                n_insertion_and_deletion_and_substitution = run_data["results"]["alignment_stats"]["counts_insertion_and_deletion_and_substitution"][
                    ref_name
                ]
