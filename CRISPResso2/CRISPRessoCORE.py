...
def get_prime_editing_guides(
    ...
    prime_editing_gap_open: alignment penalty assignment used to determine similarity of two pegRNA
        components. For prime editing the gap open is usually larger while the extension penalty is
    ...
    if this_amp_seq == prime_editing_edited_amp_seq:
        (
            best_aln_seq, best_aln_score, best_aln_mismatches, best_aln_start, best_aln_end, s1, s2,
    ...
    # otherwise, clone the coordinates from the prime_editing_edited_amp_seq
        ...
        coords_l, coords_r = CRISPRessoShared.get_alignment_coordinates(
            to_sequence=this_amp_seq, from_sequence=prime_editing_edited_amp_seq, aln_matrix=aln_matrix,
        ...
        this_mismatches = CRISPRessoShared.get_sgRNA_mismatch_vals(
            this_amp_seq, prime_editing_edited_amp_seq, pe_start_loc, pe_end_loc, coords_l, coords_r,
    ...
    if not prime_editing_pegRNA_spacer_seq:
        raise CRISPRessoShared.BadParameterException(
    ...
    if this_amp_seq == ref0_seq:
        (
            ...
            pegRNA_spacer_seq, ref0_seq, aln_matrix, prime_editing_gap_open, prime_editing_gap_extend,
        )
        pe_guides.append(best_aln_seq)
    ...
    # otherwise, clone the coordinates from the ref0 amplicon
        ...
        coords_l, coords_r = CRISPRessoShared.get_alignment_coordinates(
            to_sequence=this_amp_seq, from_sequence=ref0_seq, aln_matrix=aln_matrix,
            needleman_wunsch_gap_open=needleman_wunsch_gap_open, needleman_wunsch_gap_extend=needleman_wunsch_gap_extend,
        )
        ...
        pe_orig_guide_seqs.append(prime_editing_pegRNA_spacer_seq)
        rev_coords_l, rev_coords_r = CRISPRessoShared.get_alignment_coordinates(
            to_sequence=ref0_seq, from_sequence=this_amp_seq, aln_matrix=aln_matrix,
            needleman_wunsch_gap_open=needleman_wunsch_gap_open, needleman_wunsch_gap_extend=needleman_wunsch_gap_extend,
        )
        this_mismatches = CRISPRessoShared.get_sgRNA_mismatch_vals(
            this_amp_seq, ref0_seq, r0_start_loc, r0_end_loc, coords_l, coords_r, rev_coords_l, rev_coords_r,
        )
        this_mismatches += [coords_l[i] for i in best_aln_mismatches]  # add mismatches to original sequence
        pe_guide_mismatches.append(this_mismatches)

        pe_guide_names.append('PE spacer sgRNA')
        # if there are indels in this amplicon between the end of the guide and the nicking center, adjust the center
    ...
    if prime_editing_nicking_guide_seq:
        ...
        # nicking guide is found in the reverse_complement of the first amplicon, may be modified in
        # the other amplicons
        if this_amp_seq == ref0_seq:
            ...
            (
                ...
                nicking_guide_seq, rc_ref0_seq, aln_matrix, prime_editing_gap_open, prime_editing_gap_extend,
            )
            if nicking_guide_seq not in rc_ref0_seq:
                warn(
                    'The given prime editing nicking guide is not found in the reference sequence. '
                    'Using the best match: ' + str(best_aln_seq),
                )
            pe_guides.append(best_aln_seq)
        ...
        # otherwise, clone the coordinates from the ref0 amplicon
            ...
            r0_end_loc = match.end()
            coords_l, coords_r = CRISPRessoShared.get_alignment_coordinates(
                to_sequence=rc_this_amp_seq, from_sequence=rc_ref0_seq, aln_matrix=aln_matrix,
            ...
            pe_orig_guide_seqs.append(prime_editing_nicking_guide_seq)
            rev_coords_l, rev_coords_r = CRISPRessoShared.get_alignment_coordinates(
                to_sequence=rc_ref0_seq, from_sequence=rc_this_amp_seq, aln_matrix=aln_matrix,
                needleman_wunsch_gap_open=needleman_wunsch_gap_open,
                needleman_wunsch_gap_extend=needleman_wunsch_gap_extend,
            )
            this_mismatches = CRISPRessoShared.get_sgRNA_mismatch_vals(
                rc_this_amp_seq, rc_ref0_seq, r0_start_loc, r0_end_loc, coords_l, coords_r,
                rev_coords_l, rev_coords_r,
            )
            this_mismatches += [coords_l[i] for i in best_aln_mismatches]  # add mismatches to original sequence
...
#                amplicon_name_arr[0] = 'Unedited'
        ...
        for idx, seq in enumerate(amplicon_seq_arr):
            ...
            if args.prime_editing_pegRNA_extension_seq:
                ...
                (
                    ...
                    this_seq, amplicon_seq_arr[0], prime_editing_edited_amp_seq,
                    args.prime_editing_pegRNA_extension_seq, prime_editing_extension_seq_dna,
                    args.prime_editing_pegRNA_spacer_seq, args.prime_editing_nicking_guide_seq,
                    args.prime_editing_pegRNA_extension_quantification_window_size,
                    nicking_qw_center, nicking_qw_size, aln_matrix,
                    args.needleman_wunsch_gap_open, args.needleman_wunsch_gap_extend,
                    args.prime_editing_gap_open_penalty, args.prime_editing_gap_extend_penalty,
                )
        ...
        if args.bam_input and args.trim_sequences:
            ...
            if not args.crispresso_merge:
                ...
                fastp_cmd = (
                    '{command} -i {r1} -I {r2} --out1 {unmerged1} --out2 {unmerged2} '
                    '--thread {num_threads} --json {json_report} --html {html_report} '
                    '--overlap_len_require {min_overlap} {options} >> {log} 2>&1'
                ).format(
                    command=args.fastp_command,
                    r1=args.fastq_r1,
                    r2=args.fastq_r2,
