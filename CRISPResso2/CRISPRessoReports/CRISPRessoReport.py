...
def add_fig_if_exists(
    fig, fig_name, fig_root, fig_title, fig_caption, fig_data, amplicon_fig_names, amplicon_figures, crispresso_folder, d3_nuc_quilt_names
):
...
def assemble_figs(run_data, crispresso_folder):
    ...
    for fig in ["1a", "1b", "1c", "1d", "1e", "5a", "6a", "8a", "11c"]:
        ...
        if fig_name + "_root" in run_data["results"]["general_plots"]:
            add_fig_if_exists(
                fig,
                fig_name,
                run_data["results"]["general_plots"][fig_name + "_root"],
                "Figure " + fig,
    ...
    for amplicon_name in run_data["results"]["ref_names"]:
        ...
        for fig in ["2a", "3a", "3b", "4a", "4b", "4c", "4d", "4e", "4f", "4g", "5", "6", "7", "8", "10a", "10b", "10c", "10h", "11a"]:
            fig_name = "plot_" + fig
            if fig_name + "_root" in run_data["results"]["refs"][amplicon_name]:
                add_fig_if_exists(
                    fig,
                    fig_name,
                    run_data["results"]["refs"][amplicon_name][fig_name + "_root"],
                    "Figure " + fig_name,
            ...
            if "plot_" + fig + "_roots" in run_data["results"]["refs"][amplicon_name]:
                for idx, plot_root in enumerate(run_data["results"]["refs"][amplicon_name]["plot_" + fig + "_roots"]):
                    ...
                    add_fig_if_exists(
                        ...
                        fig_name,
                        plot_root,
                        "Figure " + fig_name + " sgRNA " + str(idx + 1),
...
def make_multi_report_from_folder(
    crispresso2_info, names_arr, report_name, crispresso_report_file, folder, _root, crispresso_tool, logger, display_names=None
):
...
def make_multi_report(
    ...
    keys_and_default_types = [("names", list), ("htmls", dict), ("titles", list), ("labels", dict), ("datas", dict), ("divs", dict)]
