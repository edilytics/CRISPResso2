...
def main():
    ...
    try:
        ...
        parser.add_argument(
            "-p", "--prefix", action="append", help="Prefix for CRISPResso folders to aggregate (may be specified multiple times)", default=[]
        )
        ...
        parser.add_argument(
            "--min_reads_for_inclusion", help="Minimum number of reads for a run to be included in the run summary", type=int, default=0
        )
        ...
        parser.add_argument(
            "--max_samples_per_summary_plot",
            type=int,
            help=(
                "Maximum number of samples on each page of the pdf report plots. "
                "If this number gets above ~150, they will be too big for matplotlib."
            ),
            default=150,
        )
        parser.add_argument(
...
|__________________________________|
        ...
        if successfully_imported_count > 0:
            ...
            for crispresso2_folder in crispresso2_folders:
                ...
                if crispresso2_folder_infos[crispresso2_folder]["running_info"]["args"].place_report_in_output_folder:
                    this_sub_html_file = os.path.join(
                        crispresso2_folder, crispresso2_folder_infos[crispresso2_folder]["running_info"]["report_filename"]
                    )
                ...
                for ref_name in run_data["results"]["ref_names"]:
                    if "plot_2a_root" in run_data["results"]["refs"][ref_name]:
                        ...
                        quilt_plots_to_show[this_folder_name + " " + ref_name] = {
                            "href": os.path.abspath(this_sub_html_file),
            ...
            with open(_jp("CRISPRessoAggregate_amplicon_information.txt"), "w", encoding="utf-8") as outfile:
                ...
                for amplicon in all_amplicons:
                    outfile.write(
                        "\t".join([amplicon_names[amplicon], str(amplicon_counts[amplicon]), ";".join(amplicon_sources[amplicon]), amplicon]) + "\n"
                    )
            ...
            for amplicon_index, amplicon_seq in enumerate(all_amplicons):
                ...
                for crispresso2_folder in crispresso2_folders:
                    ...
                    if "nuc_freq_filename" not in run_data["results"]["refs"][run_amplicon_name]:
                        info(
                            "Skipping the amplicon '%s' in folder '%s'. Cannot find nucleotide information." % (run_amplicon_name, crispresso2_folder)
                        )
                        continue
                    ...
                    if this_amp_total_reads < args.min_reads_for_inclusion:
                        info(
                            ...
                            % (run_amplicon_name, crispresso2_folder, str(this_amp_total_reads), args.min_reads_for_inclusion)
                        )
                        continue
                ...
                if amp_found_count == 0:
                    ...
                    crispresso2_info["results"]["refs"][amplicon_name]["nucleotide_frequency_summary_filename"] = os.path.basename(
                        nucleotide_frequency_summary_filename
                    )
                    crispresso2_info["results"]["refs"][amplicon_name]["nucleotide_percentage_summary_filename"] = os.path.basename(
                        nucleotide_percentage_summary_filename
                    )

                    crispresso2_info["results"]["refs"][amplicon_name]["modification_frequency_summary_filename"] = os.path.basename(
                        modification_frequency_summary_filename
                    )
                    crispresso2_info["results"]["refs"][amplicon_name]["modification_percentage_summary_filename"] = os.path.basename(
                    ...
                    if guides_all_same and consensus_guides != []:
                        ...
                        for idx, sgRNA in enumerate(consensus_guides):
                            ...
                            sub_nucleotide_frequency_summary_df = pd.concat(
                                [
                                    sub_nucleotide_frequency_summary_df.iloc[:, 0:2],
                                    sub_nucleotide_frequency_summary_df.iloc[:, 2:].apply(pd.to_numeric),
                                ],
                                axis=1,
                            )
                            ...
                            if not args.suppress_plots and this_number_samples < args.max_samples_per_summary_plot:
                                ...
                                crispresso2_info["results"]["general_plots"]["summary_plot_datas"][plot_name] = [
                                    (amplicon_name + " nucleotide frequencies", os.path.basename(nucleotide_frequency_summary_filename)),
                                    (amplicon_name + " modification frequencies", os.path.basename(modification_frequency_summary_filename)),
                                ]
                        ...
                        if not args.suppress_plots:  # and this_number_samples < 500: # plot the whole region
                            ...
                            nrow_per_sample_nucs = (
                                nucleotide_percentage_summary_df.shape[0] / this_number_samples
                            )  # calculate number of rows per sample for subsetting the tables
                            ...
                            for sample_start_ind in range(0, this_number_samples, args.max_samples_per_summary_plot):
                                ...
                                crispresso2_info["results"]["general_plots"]["summary_plot_titles"][plot_name] = (
                                    "Amplicon: " + amplicon_name + this_plot_suffix
                                )
                    ...
                    if C2PRO_INSTALLED and not args.use_matplotlib and not args.suppress_plots:
                        ...
                        for modification_type in ["Insertions", "Deletions", "Substitutions"]:
                            ...
                            modification_df.columns = [
                                "{0} ({1})".format(column, position) for position, column in enumerate(modification_df.columns, 1)
                            ]
                            plot_name = "CRISPRessoAggregate_percentage_of_{0}_across_alleles_{1}_heatmap".format(
                                modification_type.lower(), amplicon_name
                            )
                            ...
                            crispresso2_info["results"]["general_plots"]["allele_modification_heatmap_plot_titles"][plot_name] = (
                                "CRISPRessoAggregate {0} Across Samples for {1}".format(
                                    modification_type,
                                    amplicon_name,
                                )
                            )
                            ...
                            plot_name = "CRISPRessoAggregate_percentage_of_{0}_across_alleles_{1}_line".format(
                                modification_type.lower(), amplicon_name
                            )
                            plot_path = "{0}.html".format(_jp(plot_name))
            ...
            header = (
                "Name\tUnmodified%\tModified%\tReads_total\tReads_aligned\tUnmodified\tModified\tDiscarded\t"
                "Insertions\tDeletions\tSubstitutions\tOnly Insertions\tOnly Deletions\tOnly Substitutions\t"
                "Insertions and Deletions\tInsertions and Substitutions\tDeletions and Substitutions\t"
            ...
            crispresso2_info["results"]["alignment_stats"]["samples_quantification_summary_by_amplicon_filename"] = os.path.basename(
            ...
            info(
                "No CRISPResso runs could be imported.\nFiles in current directory:\n\t" +
        ...
        CRISPRessoShared.write_crispresso_info(
            crispresso2Aggregate_info_file,
            crispresso2_info,
        )
