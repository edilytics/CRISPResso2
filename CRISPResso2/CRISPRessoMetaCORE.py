...
def propagate_options(cmd, options, params, paramInd):
    ...
    # # params - df from excel parser e.g. params['amplicon_name'] = ['name1','name2','name3'] where each item corresponds to a different run in
    # the batch
...
|________________________________________|
        ...
        if "fastq_r1" not in meta_params:
            raise CRISPRessoShared.BadParameterException(
                "fastq_r1 must be specified in the meta settings file. Current headings are: " + str(meta_params.columns.values)
            )
        ...
        if meta_params.drop_duplicates("name").shape[0] != meta_params.shape[0]:
            raise CRISPRessoShared.BadParameterException(
                "Sample input names must be unique. The given names are not unique: " + str(meta_params.loc[:, "name"])
            )
            ...
            if row.fastq_r1 is None:
                raise CRISPRessoShared.BadParameterException(
                    "At least one fastq file must be given as a command line parameter or 
                        or be specified in the meta settings file with the heading 'fastq_r1' (fastq_r1 on row %s '%s' is invalid)"
            ...
            if curr_amplicon_seq_str is None:
                raise CRISPRessoShared.BadParameterException(
                    "Amplicon sequence must be given as a command line parameter or 
                        or be specified in the meta settings file with the heading 'amplicon_seq' (Amplicon seq on row %s '%s' is invalid)"
            ...
            for curr_amplicon_seq in curr_amplicon_seq_str.split(","):
                ...
                if wrong_nt:
                    raise CRISPRessoShared.NTException(
                        "The amplicon sequence in row %d (%s) contains incorrect characters:%s" % (idx + 1, curr_amplicon_seq_str, " ".join(wrong_nt))
                    )
