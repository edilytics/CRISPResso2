...
increase the memory required to run CRISPResso. Can be set to 'max'.
    ...
    # #        df_comp=df_quant_1.set_index(['Name','Amplicon']).join(df_quant_2.set_index(['Name','Amplicon']),lsuffix='_%s' %
    # args.sample_1_name,rsuffix='_%s' % args.sample_2_name)
        ...
        df_comp["({0}-{1})_Unmodified%".format(sample_1_name, sample_2_name)] = (
            df_comp["Unmodified%_{0}".format(sample_1_name)] - df_comp["Unmodified%_{0}".format(sample_2_name)]
        )
        ...
        for idx, row in df_comp.iterrows():
            ...
            if row.isnull().any():
                ...
                crispresso_compare_cmd = CRISPResso_compare_to_call + ' "{0}" "{1}" -o "{2}" -n {3} -n1 "{4}" -n2 "{5}" '.format(
