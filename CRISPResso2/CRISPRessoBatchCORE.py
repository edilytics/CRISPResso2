...
def should_plot_large_plots(num_rows, c2pro_installed, use_matplotlib, large_plot_cutoff=300):
    ...
    """
    return (not use_matplotlib and c2pro_installed) or (num_rows / 6) < large_plot_cutoff
        ...
        if len(sys.argv) == 1:
            raise CRISPRessoShared.BadParameterException(
                CRISPRessoShared.format_cl_text(
                    "usage: CRISPRessoBatch  [-bs BATCH_SETTINGS]  [-n NAME]\n"
...
|_________________|
        ...
        if "fastq_r1" not in batch_params and "bam_input" not in batch_params:
            raise CRISPRessoShared.BadParameterException(
                "fastq_r1 must be specified in the batch settings file. Current headings are: " + str(batch_params.columns.values)
            )
        ...
        if batch_params.drop_duplicates("name").shape[0] != batch_params.shape[0]:
            raise CRISPRessoShared.BadParameterException(
                "Batch input names must be unique. The given names are not unique: " + str(batch_params.loc[:, "name"])
            )
            ...
            for amp_idx, curr_amplicon_seq in enumerate(curr_amplicon_seq_arr):
                ...
                if wrong_nt:
                    raise CRISPRessoShared.NTException(
                        f"The amplicon sequence in row {idx + 1} ({curr_amplicon_seq_str}) contains incorrect characters:{" ".join(wrong_nt)}"
                    )
                ...
                if curr_guide_seq_string is not None and re.match(r"(?i)^$|^nan?$", str(curr_guide_seq_string)) is None:
                    ...
                    for curr_guide_seq in guides:
                        ...
                        if wrong_nt:
                            raise CRISPRessoShared.NTException(
                                "The sgRNA sequence in row %d (%s) contains incorrect characters:%s" % (idx + 1, curr_guide_seq, " ".join(wrong_nt))
                            )
                    ...
                    (
                        ...
                        _this_sgRNA_intervals,
                        _this_sgRNA_cut_points,
                        _this_sgRNA_plot_cut_points,
                        _this_sgRNA_plot_idxs,
                        _this_sgRNA_mismatches,
                        _this_sgRNA_names,
                        _this_sgRNA_include_idxs,
                        _this_include_idxs,
                        _this_exclude_idxs,
                        ...
                        guide_qw_centers,
                        guide_qw_sizes,
                        curr_amplicon_quant_window_coordinates,
                        row.exclude_bp_from_left,
                        row.exclude_bp_from_right,
                        row.plot_window_size,
                        guide_plot_cut_points,
        ...
        for idx, row in batch_params.iterrows():
            ...
            crispresso_cmd = (
                args.crispresso_command + f' -o "{OUTPUT_DIRECTORY}" --name {batch_name}'
            )
        ...
        CRISPRessoMultiProcessing.run_crispresso_cmds(
            crispresso_cmds,
            n_processes_for_batch,
            "batch",
            args.skip_failed,
            start_end_percent=[10, 90]
        )
        ...
        for amplicon_index, amplicon_seq in enumerate(all_amplicons):
            ...
            for idx, row in batch_params.iterrows():
                ...
                if this_amp_total_reads < args.min_reads_for_inclusion:
                    info(
                        ...
                        % (batch_amplicon_name, folder_name, str(this_amp_total_reads), args.min_reads_for_inclusion)
                    )
                    continue
            ...
            if amp_found_count == 0:
                ...
                nucleotide_frequency_summary_df = pd.concat(
                    [nucleotide_frequency_summary_df.iloc[:, 0:2], nucleotide_frequency_summary_df.iloc[:, 2:].apply(pd.to_numeric)], axis=1
                )
                ...
                nucleotide_percentage_summary_df = pd.concat(
                    [nucleotide_percentage_summary_df.iloc[:, 0:2], nucleotide_percentage_summary_df.iloc[:, 2:].apply(pd.to_numeric)], axis=1
                )
                ...
                modification_frequency_summary_df = pd.concat(
                    [modification_frequency_summary_df.iloc[:, 0:2], modification_frequency_summary_df.iloc[:, 2:].apply(pd.to_numeric)], axis=1
                )
                ...
                modification_percentage_summary_df = pd.concat(
                    [modification_percentage_summary_df.iloc[:, 0:2], modification_percentage_summary_df.iloc[:, 2:].apply(pd.to_numeric)], axis=1
                )
                ...
                if guides_all_same and consensus_guides != []:
                    ...
                    for idx, sgRNA in enumerate(consensus_guides):
                        ...
                        sub_nucleotide_frequency_summary_df = pd.concat(
                            [sub_nucleotide_frequency_summary_df.iloc[:, 0:2], sub_nucleotide_frequency_summary_df.iloc[:, 2:].apply(pd.to_numeric)],
                            axis=1,
                        )
                        sub_nucleotide_frequency_summary_filename = _jp(
                            amplicon_plot_name + "Nucleotide_frequency_summary_around_sgRNA_" + sgRNA + ".txt"
                        )
                        ...
                        sub_nucleotide_percentage_summary_df = pd.concat(
                            [
                                sub_nucleotide_percentage_summary_df.iloc[:, 0:2],
                                sub_nucleotide_percentage_summary_df.iloc[:, 2:].apply(pd.to_numeric),
                            ],
                            axis=1,
                        )
                        sub_nucleotide_percentage_summary_filename = _jp(
                            amplicon_plot_name + "Nucleotide_percentage_summary_around_sgRNA_" + sgRNA + ".txt"
                        )
                        ...
                        sub_modification_percentage_summary_df = pd.concat(
                            [
                                sub_modification_percentage_summary_df.iloc[:, 0:2],
                                sub_modification_percentage_summary_df.iloc[:, 2:].apply(pd.to_numeric),
                            ],
                            axis=1,
                        )
                        sub_modification_percentage_summary_filename = _jp(
                            amplicon_plot_name + "Modification_percentage_summary_around_sgRNA_" + sgRNA + ".txt"
                        )
                        sub_modification_percentage_summary_df.to_csv(sub_modification_percentage_summary_filename, sep="\t", index=None)

                        if (
                            not args.suppress_plots
                            and not args.suppress_batch_summary_plots
                            and should_plot_large_plots(sub_nucleotide_percentage_summary_df.shape[0], C2PRO_INSTALLED, args.use_matplotlib)
                        ):
                            # plot for each guide
                            ...
                            this_window_nuc_pct_quilt_plot_name = _jp(
                                amplicon_plot_name.replace(".", "") + "Nucleotide_percentage_quilt_around_sgRNA_" + sgRNA
                            )
                            nucleotide_quilt_input = {
                                ...
                                if not args.use_matplotlib and C2PRO_INSTALLED
                                else this_window_nuc_pct_quilt_plot_name,
                            ...
                            crispresso2_info["results"]["general_plots"]["summary_plot_titles"][plot_name] = (
                                "sgRNA: " + sgRNA + " Amplicon: " + amplicon_name
                            )
                            crispresso2_info["results"]["general_plots"]["summary_plot_labels"][plot_name] = (
                                "Composition of each base around the guide " + sgRNA + " for the amplicon " + amplicon_name
                            )
                            crispresso2_info["results"]["general_plots"]["summary_plot_datas"][plot_name] = [
                                ("Nucleotide frequencies", os.path.basename(nucleotide_frequency_summary_filename)),
                                ("Modification frequencies", os.path.basename(modification_frequency_summary_filename)),
                            ]

                            if args.base_editor_output and should_plot_large_plots(
                                sub_nucleotide_percentage_summary_df.shape[0], False, args.use_matplotlib
                            ):
                                ...
                                crispresso2_info["results"]["general_plots"]["summary_plot_labels"][plot_name] = (
                                    ...
                                    + " conversion rates around the guide "
                                    + sgRNA
                                    + " for the amplicon "
                                    + amplicon_name
                                )
                                crispresso2_info["results"]["general_plots"]["summary_plot_datas"][plot_name] = [
                                    ("Nucleotide frequencies around sgRNA", os.path.basename(sub_nucleotide_frequency_summary_filename)),
                    ...
                    if (
                        not args.suppress_plots
                        and not args.suppress_batch_summary_plots
                        and should_plot_large_plots(nucleotide_percentage_summary_df.shape[0], C2PRO_INSTALLED, args.use_matplotlib)
                        ...
                        nucleotide_quilt_input = {
                            ...
                            if not args.use_matplotlib and C2PRO_INSTALLED
                            else this_nuc_pct_quilt_plot_name,
                            "save_also_png": save_png,
                        ...
                        if len(amplicon_names) == 1:
                            crispresso2_info["results"]["general_plots"]["summary_plot_titles"][plot_name] = ""
                        crispresso2_info["results"]["general_plots"]["summary_plot_labels"][plot_name] = (
                            "Composition of each base for the amplicon " + amplicon_name
                        )
                        crispresso2_info["results"]["general_plots"]["summary_plot_datas"][plot_name] = [
                            ("Nucleotide frequencies", os.path.basename(nucleotide_frequency_summary_filename)),
                            ("Modification frequencies", os.path.basename(modification_frequency_summary_filename)),
                        ...
                        if args.base_editor_output and should_plot_large_plots(nucleotide_percentage_summary_df.shape[0], False, args.use_matplotlib):
                            ...
                            crispresso2_info["results"]["general_plots"]["summary_plot_titles"][plot_name] = ""
                            crispresso2_info["results"]["general_plots"]["summary_plot_labels"][plot_name] = (
                                args.conversion_nuc_from + "->" + args.conversion_nuc_to + " conversion rates for the amplicon " + amplicon_name
                            )
                            crispresso2_info["results"]["general_plots"]["summary_plot_datas"][plot_name] = [
                                ("Nucleotide frequencies", os.path.basename(nucleotide_frequency_summary_filename)),
                                ("Modification frequencies", os.path.basename(modification_frequency_summary_filename)),
                ...
                if C2PRO_INSTALLED and not args.use_matplotlib and not args.suppress_plots and not args.suppress_batch_summary_plots:
                    ...
                    for modification_type in ["Insertions", "Deletions", "Substitutions"]:
                        modification_df = modification_frequency_summary_df[modification_frequency_summary_df["Modification"] == modification_type]
                        modification_df.index = [
                            "{0} ({1})".format(batch, batch_index)
                            for batch_index, batch in enumerate(
                                modification_df["Batch"],
                                1,
                            )
                        ]
                        ...
                        modification_df.columns = ["{0} ({1})".format(column, position) for position, column in enumerate(modification_df.columns, 1)]
                        ...
                        crispresso2_info["results"]["general_plots"]["allele_modification_heatmap_plot_titles"][plot_name] = (
                            "CRISPRessoBatch {0} Across Samples for {1}".format(
                                modification_type,
                                amplicon_name,
                            )
                        )
                        ...
                        crispresso2_info["results"]["general_plots"]["allele_modification_line_plot_titles"][plot_name] = (
                            "CRISPRessoBatch {0} Across Samples for {1}".format(
                                modification_type,
                                amplicon_name,
                            )
        ...
        with open(_jp("CRISPRessoBatch_quantification_of_frameshift_splicing.txt"), "w", encoding="utf-8") as outfile:
            ...
            for idx, row in batch_params.iterrows():
                ...
                for ref_name in run_data["results"]["ref_names"]:
                    ...
                    count_modified_frameshift = (
                        run_data["results"]["alignment_stats"]["counts_modified_frameshift"][ref_name]
                    )
                    count_modified_non_frameshift = (
                        run_data["results"]["alignment_stats"]["counts_modified_non_frameshift"][ref_name]
                    )
                    count_non_modified_non_frameshift = (
                        run_data["results"]["alignment_stats"]["counts_non_modified_non_frameshift"][ref_name]
                    )
                    count_splicing_sites_modified = (
                        run_data["results"]["alignment_stats"]["counts_splicing_sites_modified"][ref_name]
                    )
                        ...
                        "\t".join(
                            [
                                ...
                                for x in [
                                    ...
                                    count_non_modified_non_frameshift,
                                    count_splicing_sites_modified,
                                    count_splicing_sites_unmodified,
                                ]
                            ]
                        )
                        + "\n"
        ...
        if not args.suppress_report:
            ...
            CRISPRessoReport.make_batch_report_from_folder(
                report_name, crispresso2_info, OUTPUT_DIRECTORY, _ROOT, logger
            )
