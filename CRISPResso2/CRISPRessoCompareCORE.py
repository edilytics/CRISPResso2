...
|___________________________|
        ...
        if C2PRO_INSTALLED:
            ...
            # Still need sig_counts for the summary file below
            ...
            for amplicon_name in amplicon_names_in_both:
                ...
                # Load per-amplicon data onto context scope fields
                plot_context.amplicon_name = amplicon_name
                plot_context.profile_1 = parse_profile(amplicon_info_1[amplicon_name]['quantification_file'])
                plot_context.profile_2 = parse_profile(amplicon_info_2[amplicon_name]['quantification_file'])

                sig_counts[amplicon_name] = {}
                sig_counts_quant_window[amplicon_name] = {}

                try:
                    assert np.all(plot_context.profile_1[:, 0] == plot_context.profile_2[:, 0])
                except:
                    raise DifferentAmpliconLengthException('Different amplicon lengths for the two amplicons.')

                plot_context.cut_points = run_info_1['results']['refs'][amplicon_name]['sgRNA_cut_points']
                plot_context.sgRNA_intervals = run_info_1['results']['refs'][amplicon_name]['sgRNA_intervals']
                ...
                plot_context.quant_windows_1 = run_info_1['results']['refs'][amplicon_name]['include_idxs']
                plot_context.quant_windows_2 = run_info_2['results']['refs'][amplicon_name]['include_idxs']

                amplicon_plot_name = amplicon_name + "."
                if len(amplicon_names_in_both) == 1 and amplicon_name == "Reference":
                    amplicon_plot_name = ""

                # Plot 2: Modification positions (×4 mod types)
                for mod in ['Insertions', 'Deletions', 'Substitutions', 'All_modifications']:
                    ...
                    crispresso2_info['results']['general_plots']['summary_plot_names'].append(plot_name)
                    crispresso2_info['results']['general_plots']['summary_plot_titles'][plot_name] = positions_data['mod_name'] + ' locations'
                    crispresso2_info['results']['general_plots']['summary_plot_labels'][plot_name] = positions_data['mod_name'] + ' location comparison for amplicon ' + amplicon_name + '; Top: percent difference; Bottom: p-value.'
                    crispresso2_info['results']['general_plots']['summary_plot_datas'][plot_name] = [(positions_data['mod_name'] + ' quantification', os.path.basename(mod_filename))]

                    sig_counts[amplicon_name][mod] = positions_data['sig_count']
                    sig_counts_quant_window[amplicon_name][mod] = positions_data['sig_count_quant_window']
                ...
                # Load allele DataFrames and build pairs list
                ...
                for allele_file_1, allele_file_2 in matching_allele_files:
                    ...
                    plot_context.allele_pairs.append((allele_file_1, allele_file_2, df1, df2))
                ...
                for pair_idx in range(len(plot_context.allele_pairs)):
                    ...
                    allele_data['merged_df'].to_csv(allele_comparison_file, sep="\t", index=None)
                    ...
                    label_suffix = (
                        ' Nucleotides are indicated by unique colors (A = green; C = red; G = yellow; T = purple). '
                        'Substitutions are shown in bold font. Red rectangles highlight inserted sequences. '
                        ...
                        + sample_1_name + ' followed by the values for ' + sample_2_name + '.'
                    )

                    # Top (enriched in sample_1)
