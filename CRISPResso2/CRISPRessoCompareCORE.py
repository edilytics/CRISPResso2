...
|___________________________|
        ...
        OUTPUT_DIRECTORY = "CRISPRessoCompare_on_{0}".format(
            normalize_name(
                args.name,
        ...
        for amplicon_name in amplicon_names_in_both:
            ...
            crispresso2_info["results"]["general_plots"]["summary_plot_labels"][plot_name] = (
                "Figure 1: Comparison for amplicon "
                + amplicon_name
                + "; Left: Percentage of modified and unmodified reads in each sample; Right: relative percentage of modified and unmodified reads"
            )
            ...
            for mod in ["Insertions", "Deletions", "Substitutions", "All_modifications"]:
                ...
                crispresso2_info["results"]["general_plots"]["summary_plot_labels"][plot_name] = (
                    mod_name + " location comparison for amplicon " + amplicon_name + "; Top: percent difference; Bottom: p-value."
                )
                crispresso2_info["results"]["general_plots"]["summary_plot_datas"][plot_name] = [
            ...
            for allele_file_1, allele_file_2 in matching_allele_files:
                ...
                # otherwise figure out which sgRNA was used for this comparison
                    ...
                    for sgRNA_interval, cut_point in zip(sgRNA_intervals, cut_points, strict=False):
                        ...
                        if sgRNA_seq in allele_file_1:
                            ...
                            ref_seq_around_cut = consensus_sequence[
                                max(0, this_cut_point - args.offset_around_cut_to_plot + 1) : min(
                                    seq_len, cut_point + args.offset_around_cut_to_plot + 1
                                )
                            ]
                            break
                ...
                merged = pd.merge(
                    ...
                    suffixes=("_" + sample_1_name, "_" + sample_2_name),
                    how="outer",
                )
                ...
                merged["each_LFC"] = np.log2(
                    ((merged["%Reads_" + sample_1_name] + lfc_error) / (merged["%Reads_" + sample_2_name] + lfc_error)).astype(float)
                ).replace([np.inf, np.nan], 0)
                merged = merged.sort_values(
                    ["%Reads_" + sample_1_name, "Reference_Sequence", "n_deleted", "n_inserted", "n_mutated"], ascending=False
                )
                ...
                crispresso2_info["results"]["general_plots"]["summary_plot_labels"][plot_name] = (
                    ...
                    + sample_2_name
                    + ". Alleles are sorted for enrichment in "
                    + sample_1_name
                    + "."
                )
                crispresso2_info["results"]["general_plots"]["summary_plot_datas"][plot_name] = [
